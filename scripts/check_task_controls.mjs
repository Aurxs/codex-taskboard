...
try {
  ...
  await ui.getByRole('checkbox', { name: '已取消', exact: true }).check();
  await page.keyboard.press('Escape');
  const canceled = ui.locator('.board-column.status-canceled');
  ...
  await ui.getByRole('button', { name: '显示选项卡', exact: true }).click();
  await ui.getByRole('checkbox', { name: '已取消', exact: true }).uncheck();
  await page.keyboard.press('Escape');
  ...
  console.log('passed: floating pickers, no composer reflow, keyboard/outside dismissal, narrow bounds, canceled column populated/empty/hide, model/dependency submission');
