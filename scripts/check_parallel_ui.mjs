...
try {
  ...
  await more.getByRole('button', { name: /^模型：/ }).click();
  await more.getByRole('option', { name: 'Example model', exact: true }).click();
  await more.getByRole('button', { name: /^推理强度：/ }).click();
  await more.getByRole('option', { name: '高 · high', exact: true }).click();
  ...
  await more.getByLabel('搜索参数', { exact: true }).fill('模型');
  ...
  await ui.getByRole('button', { name: 'Add subtask', exact: true }).waitFor();
  await ui.getByRole('button', { name: 'More', exact: true }).click();
