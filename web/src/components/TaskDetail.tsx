...
import type { Interaction, Task, TaskPriority, TaskStatus } from "../types";
...
export function TaskDetail({
  ...
  tasks: Task[];
  onBack: () => void;
  onUpdate: (task: Task, changes: Partial<Pick<Task, "title" | "description" | "priority" | "model" | "reasoningEffort">>) => Promise<Task | null>;
  ...
  async function save(changes: Partial<Pick<Task, "title" | "description" | "priority" | "model" | "reasoningEffort">>) {
    setSaving(true);
    try {
        ...
        <div className="issue-detail-layout">
          <div className="issue-detail-main">
            ...
            <section className="activity-section">
              ...
              <div className="activity-stream">
                ...
                {current.lastMessage && <div className="activity-entry"><span className="activity-rail-icon">✦</span><p><strong>Codex</strong> {current.lastMessage}</p><time>{relativeTime(current.updatedAt)}</time></div>}
                {(current.activity ?? []).slice(-12).map((item, index) => <div className="activity-entry" key={item.id ?? `${item.createdAt}-${index}`}><span className="activity-rail-icon">{item.kind === "command" ? "$" : item.kind === "file" ? "↗" : "·"}</span><p><strong>{item.kind === "command" ? "命令" : item.kind === "file" ? "文件变化" : item.kind === "error" ? "错误" : "消息"}</strong> {item.message ?? item.summary ?? item.detail ?? ""}</p><time>{relativeTime(item.createdAt)}</time></div>)}
                {current.runs?.map((run, index) => <div className="activity-entry" key={run.id ?? `run-${index}`}><span className="activity-rail-icon"><LinearIcon name="terminal" /></span><p><strong>TaskRun</strong> {run.summary ?? run.phase ?? run.state ?? "运行记录"}</p><time>{relativeTime(run.updatedAt ?? run.createdAt)}</time></div>)}
          ...
          <aside className="issue-properties">
            ...
            <div className="detail-primary-actions">
              {current.status === "todo" && <button className="detail-open-thread-action" type="button" disabled={!current.ready || saving} onClick={() => void onAction(current, "run")}><LinearIcon name="play" />{current.ready ? "立即交给 Codex" : "等待前置任务完成"}</button>}
