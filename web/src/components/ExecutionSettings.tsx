...
import { DependencyPickerPanel } from "./DependencyPicker";
...
export function ExecutionSettings({ value, onChange, disabled = false, workspaceLocked = false, isChild = false, errorMessage, defaultTarget = null, scopeSummary, primaryControls = false, dependencies, toolbarAction }: {
  primaryControls?: boolean; toolbarAction?: ReactNode;
  dependencies?: { candidates: Task[]; value: string[]; onChange: (ids: string[]) => void; disabled?: boolean };
  ...
  const reasoningAnchor = useRef<HTMLButtonElement>(null);
  const [primaryPage, setPrimaryPage] = useState<PrimaryField | null>(null);
  ...
  useEffect(() => {
    if (errorMessage && /model|branch|scope|模型|推理|分支|范围|目标/i.test(errorMessage)) {
      ...
      setPrimaryPage(primaryControls && (field === "model" || field === "reasoningEffort") ? field : null);
  ...
  const rows: { key: Page; label: string; text: string; changed: boolean; locked: boolean; visible: boolean }[] = [
    { key: "model", label: t("模型"), text: value.model || modelDefault, changed: !!value.model, locked: false, visible: !primaryControls },
    { key: "reasoningEffort", label: t("推理强度"), text: value.reasoningEffort ? LABELS[value.reasoningEffort] || value.reasoningEffort : value.model ? t("模型默认") : modelDefault, changed: !!value.reasoningEffort, locked: !value.model, visible: !primaryControls },
    { key: "blockedBy", label: t("阻塞于"), text: String(dependencies?.value.length || t("无")), changed: !!dependencies?.value.length, locked: !!dependencies?.disabled, visible: !!dependencies },
  ...
  function enter(field: Page) {
    setPage(field);
    if (field === "blockedBy") return;
  ...
  const popupAnchor = primaryPage === "model" ? modelAnchor : primaryPage === "reasoningEffort" ? reasoningAnchor : anchor;
  const popupLabel = primaryPage ? rows.find(row => row.key === primaryPage)!.label : t("更多设置");
  function close() {
    setOpen(false);
    popupAnchor.current?.focus();
  }
  function togglePrimary(field: PrimaryField) {
    setOpen(!(open && primaryPage === field));
    setPrimaryPage(field);
    setPage(field);
    setQuery("");
  }
  return <>
    {primaryControls && <>
      {(["model", "reasoningEffort"] as const).map(field => {
        const row = rows.find(row => row.key === field)!;
        const text = field === "model" ? selected?.displayName || row.text : row.text;
        const label = value[field] ? text : row.label;
        return <div className="task-property-picker" key={field}>
          <button ref={field === "model" ? modelAnchor : reasoningAnchor} type="button" className="property-control task-property-trigger"
            aria-label={row.label} title={`${row.label}：${text}`} disabled={disabled || row.locked} aria-haspopup="listbox" aria-expanded={open && primaryPage === field}
            onClick={() => togglePrimary(field)}><span className="task-property-trigger-label">{label}</span><LinearIcon name="chevronDown" className="picker-chevron" /></button>
        </div>;
      })}
    </>}
    {toolbarAction}
    <div className="execution-settings compact-settings">
    <button ref={anchor} type="button" className="property-control more-settings-trigger" aria-label={t("更多")} aria-haspopup="dialog" aria-expanded={open && !primaryPage}
      onClick={() => { setOpen(!(open && !primaryPage)); setPrimaryPage(null); setPage(null); setQuery(""); }}><span>{t("更多")}</span>{errorMessage && <span className="settings-error-dot" aria-label={t("设置需要处理")}>!</span>}<LinearIcon name="chevronDown" className="picker-chevron" /></button>
    <FloatingPopover open={open} anchor={popupAnchor} onClose={() => setOpen(false)} label={popupLabel} width={primaryPage ? 300 : 350}
      onEscape={() => { if (page && !primaryPage) { setPage(null); return true; } return false; }}>
      <div className="more-settings-panel" role={primaryPage ? undefined : "dialog"} aria-label={popupLabel}>
        {page ? <>
          {!primaryPage && <div className="settings-page-heading"><button type="button" aria-label={t("返回设置列表")} onClick={() => setPage(null)}><LinearIcon name="chevronLeft" /></button><strong>{activeRow?.label}</strong></div>}
          {page === "blockedBy" && dependencies ? <DependencyPickerPanel {...dependencies} /> : page !== "blockedBy" && (choices.length > 0 ? <div className="settings-choices" role="listbox" aria-label={activeRow?.label}>
          ...
          {!primaryPage && page !== "blockedBy" && !disabled && !activeRow?.locked && (page !== "targetBranch" || !!defaultTarget) && <button className="settings-reset" type="button" onClick={() => choose(page, page === "executionMode" ? "local" : page === "targetBranch" ? defaultTarget : null)}>{t("恢复默认")}</button>}
        </> : <>
          ...
          <div className="settings-rows">{rows.filter(row => row.visible && `${row.label} ${row.text}`.toLocaleLowerCase().includes(query.toLocaleLowerCase())).map(row => <button key={row.key} type="button" data-popover-item className="settings-row" disabled={(row.key === "blockedBy" ? false : disabled) || row.locked} onClick={() => enter(row.key)} aria-label={`${row.label}：${row.text}`}>
        ...
        {workspaceLocked && page && !primaryPage && <small className="settings-note">{t("执行位置和分支已锁定，重试将沿用原会话。")}</small>}
