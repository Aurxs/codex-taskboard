...
const MONOCHROME_ICONS = new Set<TaskboardIconName>([
  "aiLauncher", "automationPlay", "breadcrumb", "dropdown", "filter", "home", "panel", "projectFolder", "search",
]);
...
export function TaskboardIcon({ name, className, ...props }: Omit<ImgHTMLAttributes<HTMLImageElement>, "alt" | "src"> & { name: TaskboardIconName }) {
  const classes = [
    "taskboard-icon",
    MONOCHROME_ICONS.has(name) ? "taskboard-icon-monochrome" : "",
    className ?? "",
  ].filter(Boolean).join(" ");
  return <img {...props} className={classes} src={TASKBOARD_ICONS[name]} alt="" aria-hidden="true" draggable={false} />;
}
