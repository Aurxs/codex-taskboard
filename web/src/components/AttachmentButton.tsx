...
export function validateAttachments(existing: { size: number }[], added: File[]) {
  ...
  if (all.length > 10 || all.some(file => file.size > 10 * 1024 * 1024) || all.reduce((sum, file) => sum + file.size, 0) > 20 * 1024 * 1024) {
    throw new Error("最多 10 个附件，单个最多 10 MB，合计最多 20 MB。");
  }
  if (added.some(file => !/\.(png|jpe?g|gif|webp|md|markdown|txt|pdf|pptx?|docx?|xlsx?)$/i.test(file.name))) throw new Error("仅支持图片、Markdown、文本、PDF 和 Office 文档。");
}
...
export function AttachmentButton({ count, disabled, onAdd }: { count: number; disabled: boolean; onAdd: (files: File[]) => void }) {
  ...
  return <>
    <button className="property-control attachment-trigger" type="button" disabled={disabled} onClick={() => input.current?.click()} title="添加图片、Markdown、PDF 或 Office 文档；也可在内容区域直接粘贴">添加附件 <span aria-live="polite">{count}</span></button>
