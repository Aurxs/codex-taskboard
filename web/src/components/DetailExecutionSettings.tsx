...
/** Task details keep parameter values visible while a field is being edited. */
export function DetailExecutionSettings({ value, onChange, disabled = false, workspaceLocked = false, isChild = false, errorMessage, defaultTarget = null, scopeSummary }: {
  ...
  return <section className="detail-execution-settings" aria-label={t("更多设置")}>
          <div className="settings-rows">{rows.filter(row => row.visible).map(row => <button key={row.key} type="button" className="settings-row" disabled={disabled || row.locked} onClick={() => enter(row.key)} aria-label={`${row.label}：${row.text}`} aria-expanded={page === row.key}>
        ...
        {page && <div className="detail-settings-editor">
