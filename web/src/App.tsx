...
function ProjectSwitcher({
  ...
  return (
    <div className="header-project-switcher" ref={menuRef} data-project-switcher>
      <button className="header-project-button" type="button" aria-label="切换项目" aria-haspopup="menu" aria-expanded={open} onClick={() => { setNeedle(""); setOpen((current) => !current); }}>
        <span className="project-name">{selected?.name ?? "等待 Codex 项目"}</span>
      ...
      {open && (
        <div className="header-project-menu" role="menu" aria-label="项目">
          ...
          <div className="project-menu-search">
            ...
            <label className="sr-only" htmlFor="project-menu-search-input">按名称筛选项目</label>
            <input id="project-menu-search-input" type="search" autoFocus value={needle} onChange={(event) => setNeedle(event.target.value)} placeholder="筛选项目…" />
            {needle && <button className="search-clear" type="button" aria-label="清除项目筛选" onClick={() => setNeedle("")}><LinearIcon name="close" /></button>}
          </div>
          <div className="project-menu-list">
            ...
            {filtered.length === 0 && <div className="project-menu-empty">没有匹配项目</div>}
          </div>
        </div>
      )}
    </div>
...
function Switch({ checked, label, description, onChange }: { checked: boolean; label: string; description: string; onChange: (value: boolean) => void }) {
  return (
    <div className="project-automation-switch">
      ...
      <button className={`board-setting-switch${checked ? " is-on" : ""}`} type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)}><span aria-hidden="true" /></button>
    </div>
  );
}
  ...
  return (
    <div className="project-settings-anchor no-drag" ref={rootRef}>
      <button className={`project-automation-trigger no-drag ${project.automationEnabled ? "is-active" : "is-paused"}`} type="button" aria-haspopup="dialog" aria-expanded={open} onClick={() => setOpen((current) => !current)}>
        ...
        <span>{project.automationEnabled ? "自动认领中" : "自动化"}</span>
      </button>
      {open && (
        <div className="project-automation-menu no-drag" role="dialog" aria-label="自动认领设置">
          <div className="project-automation-menu-heading"><strong>自动认领待办</strong><span className={project.automationEnabled ? "is-active" : "is-paused"}>{project.automationEnabled ? "运行中" : "已暂停"}</span></div>
          <Switch checked={project.automationEnabled} label="自动认领开关" description="就绪后自动执行" onChange={(value) => onChange("automationEnabled", value)} />
          <Switch checked={project.reviewRequired} label="人工审阅" description="完成后等你确认" onChange={(value) => onChange("reviewRequired", value)} />
          <Switch checked={project.quotaAutoResumeEnabled} label="额度恢复自动续跑" description="额度恢复后继续原 thread" onChange={(value) => onChange("quotaAutoResumeEnabled", value)} />
          <p className="project-automation-note">任务可单独选择模型和推理强度；未指定时沿用 Codex 会话设置。</p>
        </div>
      )}
    </div>
  );
...
function BoardToolbar({ search, onSearch, includeCanceled, onIncludeCanceled }: { search: string; onSearch: (value: string) => void; includeCanceled: boolean; onIncludeCanceled: (value: boolean) => void }) {
  return (
    <div className="board-toolbar">
      <div className="view-tabs" aria-label="看板视图"><button className="view-tab active" type="button" aria-pressed="true">议题看板</button></div>
      <div className="toolbar-tools">
        <div className={`search-field${search ? " has-value" : ""}`} title="搜索任务">
          <TaskboardIcon className="search-icon" name="search" />
          <input type="search" aria-label="搜索任务" value={search} onChange={(event) => onSearch(event.target.value)} placeholder="搜索任务…" />
          {!search && <kbd>/</kbd>}
          {search && <button className="search-clear" type="button" aria-label="清除搜索" onClick={() => onSearch("")}><LinearIcon name="close" /></button>}
        </div>
        <button className={`other-tasks-trigger${includeCanceled ? " is-open" : ""}`} type="button" aria-controls="canceled-tasks-panel" aria-expanded={includeCanceled} aria-pressed={includeCanceled} onClick={() => onIncludeCanceled(!includeCanceled)} title="显示已取消任务"><TaskboardIcon name="panel" /><span>已取消</span></button>
      </div>
    </div>
  );
}
...
function ErrorBanner({ message, onRetry }: { message: string; onRetry: () => void }) {
  return <div className="error-banner" role="alert"><span className="error-mark"><LinearIcon name="alert" /></span><div><strong>任务面板需要处理</strong><p>{message}</p></div><button type="button" onClick={onRetry}>重试</button></div>;
}

function NonEmbeddedMessage() {
  return <div className="app-shell embedded-required"><main className="workspace"><div className="page-empty"><TaskboardIcon name="panel" /><h2>请在 Codex 侧栏打开任务面板</h2><p>Taskboard 只作为 Codex 内嵌面板运行，不提供独立软件页面。</p></div></main></div>;
}

function HostWaitingMessage({ hasProjects }: { hasProjects: boolean }) {
  return <div className="page-empty"><TaskboardIcon name="projectFolder" /><h2>{hasProjects ? "正在同步 Codex 项目…" : "等待 Codex 项目"}</h2><p>{hasProjects ? "正在读取当前 Codex 项目。" : "请先在 Codex 中打开一个本地项目。"}</p></div>;
}
...
function EmbeddedTaskboard() {
  ...
  const performAction = useCallback(async (task: Task, action: TaskAction, feedback?: string, targetStatus?: "in_review" | "done") => {
    try {
      ...
      updateTaskInState(next);
      notify(action === "complete" ? "任务已完成" : action === "run" || action === "retry" ? "任务已提交给 Codex" : "操作已提交", "success");
      return next;
    } catch (error) {
      if (error instanceof ApiError && error.status === 409) await refreshTasks();
      notify(compactError(error), "error");
  ...
  const updateProjectSetting = useCallback(async (field: "automationEnabled" | "reviewRequired" | "quotaAutoResumeEnabled", value: boolean) => {
    ...
    try {
      ...
      notify(value ? `${field === "automationEnabled" ? "自动认领" : field === "reviewRequired" ? "人工审阅" : "额度恢复自动续跑"}已开启` : `${field === "automationEnabled" ? "自动认领" : field === "reviewRequired" ? "人工审阅" : "额度恢复自动续跑"}已关闭`, "success");
    } catch (error) {
      notify(compactError(error), "error");
    }
  ...
  async function drop(status: TaskStatus, taskId: string) {
    ...
    if (status === "done" && (task.status === "todo" || task.status === "in_review" || task.status === "in_progress")) {
      if (window.confirm(`确定将 ${task.identifier} 标记为已完成吗？`)) await performAction(task, "complete", undefined, task.status === "in_progress" ? "done" : undefined);
      return;
    }
    ...
    if (task.status === "in_progress" && status === "in_review") {
      if (window.confirm("这会中断当前 turn，并将任务送入人工审阅。继续吗？")) await performAction(task, "complete", undefined, "in_review");
      return;
    }
    if (task.status === "in_review" && status === "in_progress") {
      const feedback = window.prompt("请填写退回修改的反馈：", "");
      if (feedback?.trim()) await performAction(task, "submit_review_feedback", feedback.trim());
      return;
    }
    notify(`不能从「${STATUS_LABELS[task.status as TaskStatus] ?? task.status}」移动到「${STATUS_LABELS[status]}」。`, "error");
  }
      ...
      <main className="workspace">
        ...
        <header className="workspace-header">
          <div className="workspace-title"><div className="workspace-kicker">
            {selectedTask && <button className="detail-back-button" type="button" onClick={() => { setSelectedTaskId(null); setDetail(null); }} aria-label="返回议题看板"><LinearIcon name="chevronLeft" /></button>}
            {hostContext?.sidebarCollapsed && <button className="detail-back-button codex-sidebar-expand-button" type="button" onClick={() => postEmbeddedHostMessage({ type: "taskboard:expand-sidebar" })} aria-label="展开 Codex 侧边栏"><LinearIcon name="codexSidebarExpand" /></button>}
          ...
          <div className="header-actions">
            ...
            {selectedProject && <button className="icon-button header-create-button no-drag" type="button" onClick={() => setEditorStatus("todo")} aria-label="新建任务" title="新建任务"><LinearIcon name="plus" /></button>}
          </div>
        </header>
